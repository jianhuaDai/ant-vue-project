<template>
  <div class="duty-statis">
    <div class="duty-statis-cont">
            ...
            <a-col :span="24">
              <a-card :bodyStyle="{padding:'10px 20px 10px 10px'}">
                <a-form :form="form" :style="{'margin-top':'16px'}">
                  <a-row>
                    <a-col :span="7">
                      <a-form-item label="河流名称:" :label-col="{span:9}" :wrapper-col="{span:15}">
                        <a-input v-decorator="['river_name']"></a-input>
                      </a-form-item>
                    </a-col>
                    <a-col :span="7">
                      <a-form-item label="事件标题:" :label-col="{span:9}" :wrapper-col="{span:15}">
                  ...
                  <a-row>
                    <a-col :span="7">
                      <a-form-item label="事件来源:" :label-col="{span:9}" :wrapper-col="{span:15}">
                        <a-input v-decorator="['source']"></a-input>
                      </a-form-item>
                    </a-col>
                    <a-col :span="5" :offset="12" style="text-align:right;">
                  ...
                  >
                    ...
                    <template slot="action" slot-scope="text, record">
                      <span class="btnGroup1">
                        <a-button size="small" class="modifyClass" @click="viewDetail(record,0)">详情</a-button>&nbsp;
                        <a-button size="small" class="modifyClass" @click="viewDetail(record,1)">受理</a-button>&nbsp;
                      </span>
                    </template>
                  </a-table>
                </a-col>
      ...
      <!-- 事件受理详情 -->
      <a-modal title="事件受理详情" :visible="visible" width="45%" @cancel="handleCancel" footer>
        <a-form :form="form2">
          <a-row :gutter="8">
            <a-col :span="24">
              <h4>基本信息</h4>
          ...
          <a-row :gutter="8">
            ...
            <a-col :span="12" v-if="this.detail.type">
          ...
          <a-row v-if="handleStatus==1" :gutter="8">
            <a-col :span="12">
              <a-form-item label="事件分类:" :label-col="{span:8}" :wrapper-col="{span:12}">
                <a-select defaultValue="水资源保护" style="width: 120px" @change="handleTypeChange">
            ...
            <a-col :span="12">
              <a-form-item label="处理方式:" :label-col="{span:8}" :wrapper-col="{span:12}">
                <a-select defaultValue="待核实" style="width: 120px" @change="handleWayChange">
                  <a-select-option value="待核实">待核实</a-select-option>
                  <a-select-option value="受理分发">受理分发</a-select-option>
                  <a-select-option value="上级审批">上级审批</a-select-option>
                  <a-select-option value="需要反馈">需要反馈</a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
            <a-col :span="24">
              <a-form-item label="预处理意见:" :label-col="{span:4}" :wrapper-col="{span:16}">
          ...
          <a-row>
            <a-col :span="24" style="text-align:right;margin-top:10px;">
              <a-button type="primary" @click="submitPopup">{{confirmText}}</a-button>
...
<script>
import { getEventAccept } from '@/api/event';
  ...
  methods: {
    ...
    searchClick() {
      ...
      let reqData={
        ...
        page_size: this.pageSize,
        model: '事件受理',
        river_name:this.form.getFieldValue('river_name'),
        event_name:this.form.getFieldValue('event_name'),
        source:this.form.getFieldValue('source')
      }
      this.searchData(reqData);
    },
    // 重置按钮触发
    resertClick() {
      this.form.setFields({ river_name: '' })
      this.form.setFields({ event_name: '' })
      this.form.setFields({ source: '' })
    },
    ...
    searchData(reqData) {
      this.dataLoad = true
      getEventAccept(reqData)
        .then(res => {
          console.log('===getEventAccept success ====')
          this.dataLoad = false
        ...
        .catch(err => {
          console.log('===getEventAccept err ====')
          this.dataLoad = false
        })
    },
    // 表格新增

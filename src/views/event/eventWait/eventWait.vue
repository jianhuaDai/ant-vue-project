<template>
  <div class="duty-statis">
    <div class="duty-statis-cont">
            ...
            <a-col :span="24">
              <a-card :bodyStyle="{padding:'10px 20px 10px 10px'}">
                <a-form :form="form" :style="{'margin-top':'16px'}">
                  <a-row>
                    <a-col :span="7">
                      <a-form-item label="处理方式:" :label-col="{span:9}" :wrapper-col="{span:15}">
                        <a-select
                          defaultValue="待核实"
                          v-model="handleWay"
                          style="width: 120px"
      ...
      <!-- 待办事件详情 -->
      <a-modal title="待办事件详情" :visible="visible" width="45%" @cancel="handleCancel" footer>
        <a-form :form="form2">
          <a-row :gutter="8">
            <a-col :span="24">
              <h4>基本信息</h4>
          ...
          <a-row v-if="handleStatus==1">
            <a-col :span="24">
              <a-form-item label="处理意见:" :label-col="{span:4}" :wrapper-col="{span:16}">
...
<script>
import { getEventWait } from '@/api/event';
...
export default {
  ...
  methods: {
    ...
    searchData(reqData) {
      this.dataLoad = true
      getEventWait(reqData)
        .then(res => {
          console.log('===getEventWait success ====')
          this.dataLoad = false

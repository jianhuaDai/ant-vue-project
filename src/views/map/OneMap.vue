...
<script>
  ...
  export default {
    ...
    components: { MapView, MaskPageView },
    methods: {
      ...
      renderPointLayer (layerId, data, shape) {
        const pointLayer = new PointLayer({ name: layerId, zIndex: 11 })
          .source(data, {
            cluster: false,
            clusterOptions: { radius: 20 }
          })
          .shape(shape)
          .scale('point_count', {
            type: 'quantile'
      ...
      renderLayer (layerId, res) {
        ...
        switch (layerId) {
          ...
          case 12: {
            ...
            // this.renderPointLayer(layerId, res.data.geo_info, 'water')
            break
          }
          case 13: {
            geoData.features.forEach((v, index) => {
      ...
      sceneLoaded (scene) {
        ...
        setTimeout(() => {
          ...
          loadWFS({
            ...
            request: 'GetFeature',
            typename: 'china2019:gis_osm_water_a_free_1',
            outputFormat: 'application/json',
          ...
          loadWFS({
            ...
            cql_filter: "fclass='water' AND name LIKE '%湖' AND BBOX(the_geom,118.49029541015625,31.923027019273864,119.00665283203124,32.29177633471201)"
          }).then(res => {
            const features = res.data
            const layer = new PolygonLayer({
              name: 'lake2',
          ...
          loadWFS({
            ...
            outputFormat: 'application/json',
            // bbox: '118.5431671142578,31.777796240436775,119.14466857910156,32.210477226433824',
            cql_filter: "fclass='water' AND name LIKE '%水库' AND BBOX(the_geom,118.49029541015625,31.923027019273864,119.00665283203124,32.29177633471201)"
          }).then(res => {
            const features = res.data
            const layer = new PolygonLayer({
              name: 'dam',
          ...
          setTimeout(() => {
            loadNJWFS({
              ...
              scene.addLayer(areaBoundaryLine)
            })
          }, 500)
        }, 500)
      },
